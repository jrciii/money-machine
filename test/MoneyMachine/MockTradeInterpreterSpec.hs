  ...
  describe "mockInterpret" $ do
    it "opens market orders from strategies" $ do
      ...
      res `shouldBe`
        ...
        , M.fromList [("EUR_USD", [])]
        , [])
    it "closes a long position when sell limit of same number of units is triggered" $ do
      let openOneMarketLongAndOneTakeProfitStrategy (md, op, cp, po) =
            let (inst, _) = head $ M.toList md
                Right po = mkPendingOrder (-1000) 1.0003 "EUR_USD" SellLimit
            in if length op == 0
               then ([MarketOrder {marketOrderUnits = 1000, marketOrderInstrument = inst}], [po], [])
               else ([], [], [])
      let shouldTrade = lift $ fmap (/= 3) (modify (+ 1) >> get)
      let marketData = M.fromList [("EUR_USD", [(0.9997, 0.9999)])]
      let marketModifier = modify (fmap (fmap (\(b,a) -> (b+0.0003,a+0.0003)))) >> get
      let prog = trade openOneMarketLongAndOneTakeProfitStrategy shouldTrade marketModifier
      res <- (evalStateT (
        (evalStateT
           (execStateT
              (mockInterpret prog)
