...
clusterStratOnTick starttime market (Spread (Points spread)) commission os newTrades = do
  ...
  case (eurUsdCandles,gbpUsdCandles) of
    ...
    _ -> do
      ...
      Right (_,_,fesmas) <- ta_sma ecv 5
      Right (_,_,fgsmas) <- ta_sma gcv 5
      Right (_,_,mesmas) <- ta_sma ecv 8
      Right (_,_,mgsmas) <- ta_sma gcv 8
      Right (_,_,sesmas) <- ta_sma ecv 13
      Right (_,_,sgsmas) <- ta_sma gcv 13
